...
ggplot(train, aes(x = pclass, fill = factor(survived))) + geom_histogram(width = 0.5) +
  xlab("Pclass") +
  ylab("Total Count") +
  labs(fill = "Survived")
...
#Expand upon the relationshiop between 'Survived' and 'Pclass' by adding the new 'Title' variable to the data set and then explore a potential 3-dimensional relationship
...
#OK, age and sex seem pretty important as derived from analysis of title, let's take a look at the distribution of age over entire data set
summary(data.combined$age)

#survival rates by age, pclass
...
# Okay, appears female children may have different survival rates, could be a candidate for feature engineering later
misses.alone <- misses[which(misses$sibsp == 0 & misses$parch == 0),]
summary(misses.alone)
length(which(misses.alone$age <= 14.5))

# Move on to the sibsp variable
...
# Visualize to see if it's predictive
...
# Based on the huge number of levels ticket really isn't a factor variable, it is a string.Convert it and display first 20
data.combined$ticket <- as.character(data.combined$ticket)
data.combined$ticket[1:20]

# There's no immediately apparent structure in the data, let's see if you can find some. We'll start with taking a look at just the first char for each
...
# Ticket seems like it might be predictive, so let's drill down
ggplot(data.combined[1:891,], aes(x = ticket.first.char, fill = survived)) + geom_bar() +
facet_wrap(~pclass) +
ggtitle("Pclass") +
xlab("ticket.first.char") +
...
# Lastly, see if we get a pattern when using a combination of pclass & title
ggplot(data.combined[1:891,], aes(x = ticket.first.char, fill = survived)) + geom_bar() +
  facet_wrap(~pclass + title) +
  ggtitle("Pclass, Title") +
  xlab("ticket.first.char") +
...
# Cabin is not really a factor, make a string and the display first 100
...
ggplot(data.combined[1:891,], aes(x = cabin.multiple, fill = survived)) +   geom_bar() +
  facet_wrap(~pclass + title) +
  ggtitle("Pclass + title") +
  xlab("cabin.multiple") +
  ylab("Total Count") +
...
ggplot(data.combined[1:891,], aes(x = embarked, fill = survived)) +   geom_bar() +
  facet_wrap(~pclass + title) +
  ggtitle("Pclass, Title") +
  xlab("embarked") +
  ylab("Total Count") +
...
write.csv(submit.df, file="Titanic_Data_Final.csv", row.names = FALSE)
...
# The above is only slightly more pessimistic from the rf.5 .008 prediction, but not pessimistic enough. 
...
# 5-fold CV isn't better, so move to 3-fold CV repeated 10 times
set.seed(37596)
...
#     2 - Titles of "Master.", "Miss." & "Mrs." in 1st and 2nd class are predicted to survive at an overall accuracy 
#         rate of 94.9%.
#     3 - Titles of "Master.", "Miss." & "Mrs." in 3rd class with family sizes equal to 5,6,8,11 are predicted to
#         perish with 100% accuracy.
#     4 - Titles of "Master.", "Miss.", & "Mrs." in 3rd class with family sizes not equal to 5,6,8, or 11 are predicted
#         to survive with 59.6% accuracy.

# Both rpart and rf confirm that title is important, so let's investigate further
...
# Take a look at the higher fares
...
# Use median since close to mean and a litle higher than mean
...
#-----------------------------------------------------------------------------------
#
# Submission, scoring and final analysis
#
#-----------------------------------------------------------------------------------
...
write.csv(submit.df, file = "RPART_SUB_20170419_1.csv", row.names = FALSE)
...
write.csv(submit.df, file = "RF_SUB_20170419_2.csv", row.names = FALSE)

# If we want to improve our model, a good place to start is focusing on where things are going wrong!
...
# should align well to the defintion of mutual information.
...
# potential upside for improving(i.e., the tree predicts incorrectly for 86 adult males). 
# Let's visualize
...
# How about creating tsne features for all of the training data and using them in the model?
...
# Now conditional mutual information 
